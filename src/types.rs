...
#[derive(Debug, Clone)]
pub struct IDMClockMode {
    pub style: u8,
    pub visible_date: bool,
    pub hour24: bool,
    pub r: Option<i32>,
    pub g: Option<i32>,
    pub b: Option<i32>
...
impl IDMCommand {
    pub fn to_bytes(&self) -> Vec<u8> {
        match self {
            ...
            IDMCommand::ClockMode(clock_mode) => {
                vec![
                    8,0,6,1,
                    (clock_mode.style | if clock_mode.visible_date {128} else {0} | if clock_mode.hour24 {64} else {0}),
                    if let Some(r) = clock_mode.r { (r % 256).try_into().unwrap() } else {(255 % 256) as u8},
                    if let Some(g) = clock_mode.g { (g % 256).try_into().unwrap() } else {(255 % 256) as u8},
                    if let Some(b) = clock_mode.b { (b % 256).try_into().unwrap() } else {(255 % 256) as u8},
                ]
            }
        }
    }
